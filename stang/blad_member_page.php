...
if(isset($_GET['blad_hps_all'])){
    ...
    $blad_query_get = mysql_query("select * from user where hapus_user='$blad_hps_all'  and level='member'");    
    while($blad_get_file = mysql_fetch_array($blad_query_get)){
        if($blad_get_file['hapus_user']=="ok"){
		$blad_sql_hps_all = mysql_query("delete from user where id_user='$blad_get_file[id_user]' and level='member'");
		if($blad_sql_hps_all){
            unlink("../davinci/member/".$blad_get_file['foto']);
            header("location:blad_member_page.php");
            }
		}else{
            header("location:blad_member_page.php");
        }
        $blad_delete_all_act = mysql_query("insert into kelakuan set id_user='$blad_id', tgl_tindakan='$blad_date', tindakan='Menghapus semua data member', hapus_tindakan='ok'");
	}
...
if(isset($_GET['blad_id_lap_hps'])){
    ...
    $blad_sql_id_lap_hps = mysql_query("delete from user where id_user='$blad_id_lap_hps' and level='member'");
    $blad_delete_act = mysql_query("insert into kelakuan set id_user='$blad_id', tgl_tindakan='$blad_date', tindakan='Menghapus member : $blad_ambil_gambar[nama_user]', hapus_tindakan='ok'");
    if($blad_sql_id_lap_hps){
        unlink("../davinci/member/".$blad_ambil_gambar['foto']);
        header("location:blad_member_page.php");
    }else{        ?>
...
if(isset($_POST['blad_id_user'])){
    ...
    $blad_sql_cek      = mysql_query("select * from user where username='$blad_username' and level='member'");
...
<?php
        ...
        $blad_insert_act = mysql_query("insert into kelakuan set id_user='$blad_id', tgl_tindakan='$blad_date', tindakan='Tambah member : $blad_nama_user', hapus_tindakan='ok'");
    ...
    <div class="register">
        <div class="content-login">
            ...
            <div class="in-login" id="in-dua">
                <button type="button" class="tutup" onclick="location.href='blad_member_page.php'"><span class="glyphicon glyphicon-remove"></span></button>
                </div>    
            </div>
